...
export async function sendPasswordResetCode(email: string, code: string): Promise<void> {
  ...
  const result = await resend.emails.send({
    from: FROM,
    to: email,
    subject: `${code} is jouw doel.io wachtwoord-resetcode`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        <h2 style="color: #0e7490; margin-bottom: 8px;">Wachtwoord opnieuw instellen</h2>
        <p style="color: #374151; margin-bottom: 24px;">We hebben een verzoek ontvangen om het wachtwoord van jouw doel.io account te resetten. Gebruik de onderstaande code.</p>
        <div style="background: #f0f9ff; border: 1px solid #bae6fd; border-radius: 8px; padding: 24px; text-align: center; margin-bottom: 24px;">
          <span style="font-size: 36px; font-weight: bold; letter-spacing: 8px; color: #0e7490;">${code}</span>
        </div>
        <p style="color: #6b7280; font-size: 14px;">De code is 10 minuten geldig en kan maar &eacute;&eacute;n keer gebruikt worden.</p>
        <p style="color: #6b7280; font-size: 14px;">Heb je dit niet aangevraagd? Dan kun je deze e-mail veilig negeren.</p>
      </div>
    `,
  });
  if (result.error) {
